...
export async function POST(
  ...
  try {
    ...
    // user_menus 확인
    const { data: userMenu, error: userMenuError } = await supabase
      .from("user_menus")
      .select("*")
      .eq("user_id", user.id)
      .eq("menu_id", menuId)
    ...
    if (userMenu) {
      ...
      if (menuUpdateError) {
        throw new Error(
          `Failed to update menu like count: ${menuUpdateError.message}`
        );
      }
    } else {
      // 새로 좋아요 생성
      const { error: insertError } = await supabase.from("user_menus").insert({
        user_id: user.id,
        menu_id: menuId,
        liked: true,
      });
